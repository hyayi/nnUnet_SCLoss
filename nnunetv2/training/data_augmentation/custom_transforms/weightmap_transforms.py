...
class DownsampleSegForDSTransformWeight(BasicTransform):
    ...
    def _apply_continuous_tensor(self, tensor: torch.Tensor, mode='bilinear', **params) -> List[torch.Tensor]:
        results = []
        for s in self.ds_scales:
            if not isinstance(s, (tuple, list)):
                s = [s] * (tensor.ndim - 1)
            else:
                assert len(s) == tensor.ndim - 1

            if all(i == 1 for i in s):
                results.append(tensor)
            else:
                new_shape = [round(i * j) for i, j in zip(tensor.shape[1:], s)]
                results.append(interpolate(tensor[None], new_shape, mode=mode, align_corners=False)[0])
        return results
...
class SpatialTransformWeight(BasicTransform):
    def __init__(self,
