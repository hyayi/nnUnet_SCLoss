...
from nnunetv2.training.loss.compound_losses import DC_and_SC_loss,DC_and_CE_SCloss
...
class nnUNetTrainerDCSCLossCole(nnUNetTrainerCole):
    def _build_loss(self):
        loss = DC_and_SC_loss(
            soft_dice_kwargs={
                'batch_dice': self.configuration_manager.batch_dice,
...
class nnUNetTrainerDCCESCLossCole(nnUNetTrainerCole):
    def _build_loss(self):
        loss = DC_and_CE_SCloss({'batch_dice': self.configuration_manager.batch_dice,
                                'smooth': 1e-5, 'do_bg': False, 'ddp': self.is_ddp}, {},{'k': 2,'alpha': 1.0,'target_classes': [1]},weight_ce=1, weight_dice=1,
                                ignore_label=self.label_manager.ignore_label, dice_class=MemoryEfficientSoftDiceLoss)
