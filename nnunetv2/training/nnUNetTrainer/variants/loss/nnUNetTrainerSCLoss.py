...
class nnUNetTrainerDCCESCLossTube3(nnUNetTrainer):
    def _build_loss(self):
        loss = DC_and_CE_SCloss({'batch_dice': self.configuration_manager.batch_dice,
                                'smooth': 1e-5, 'do_bg': False, 'ddp': self.is_ddp}, {},{'k': 3,'alpha': 1.0,'target_classes': [1]},weight_ce=1, weight_dice=1,
                                ignore_label=self.label_manager.ignore_label, dice_class=MemoryEfficientSoftDiceLoss)
...
class nnUNetTrainerDCSCLossTube3(nnUNetTrainer):
    def _build_loss(self):
        loss = DC_and_SC_loss(
            soft_dice_kwargs={
                'batch_dice': self.configuration_manager.batch_dice,
