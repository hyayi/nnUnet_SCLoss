...
from nnunetv2.training.loss.compound_losses import DC_SkelREC_and_CE_loss,DC_SkelREC_loss,TopologyAwareLoss,DC_and_CE_TopologyAwareLoss
...
from nnunetv2.training.data_augmentation.custom_transforms.cole_transforms import CoLeTraTransform


class nnUNetTrainerTopologyAwareLoss(nnUNetTrainer):
    ...
    def _build_loss(self):
        ...
        topology_kwargs = {
            ...
            'iterations': 10,  # soft_skeletonize 반복 횟수
            'start_lambda': 0.1,
            'end_lambda': 1.0,
        ...
        # CE 관련 인자를 제거하고 topology_kwargs를 추가합니다.
        loss = TopologyAwareLoss(
            ...
            weight_dice=1, 
            weight_recall=self.weight_srec, # self.weight_srec을 weight_recall로 전달
    ...
    def train_step(self, batch: dict) -> dict:
        ...
        if isinstance(target, list):
            ...
            epoch = [self.current_epoch for _ in skel]
        else:
        ...
        with autocast(self.device.type, enabled=True) if self.device.type == 'cuda' else dummy_context():
            ...
            # del data
            l = self.loss(output, target, skel,epoch)
...
class nnUNetTrainerTopologyAwareLossColre(nnUNetTrainerTopologyAwareLoss):
    @staticmethod
    def get_training_transforms(
            patch_size: Union[np.ndarray, Tuple[int]],
            rotation_for_DA: RandomScalar,
...
class nnUNetTrainerTopologyAwareLossCE(nnUNetTrainerTopologyAwareLoss):
    def _build_loss(self):
        if self.label_manager.ignore_label is not None:
            warnings.warn('Support for ignore label with Skeleton Recall is experimental and may not work as expected')
        topology_kwargs = {
        ...
        loss = DC_and_CE_TopologyAwareLoss(
            soft_dice_kwargs={'batch_dice': self.configuration_manager.batch_dice, 
                            'smooth': 1e-5, 'do_bg': False, 'ddp': self.is_ddp}, 
            soft_skelrec_kwargs={'batch_dice': self.configuration_manager.batch_dice, 
            ...
            weight_ce=1, 
            weight_dice=1, 
            weight_recall=self.weight_srec, # weight_srec -> weight_recall
            dice_class=MemoryEfficientSoftDiceLoss
        )
...
class nnUNetTrainerTopologyAwareLossCEColre(nnUNetTrainerTopologyAwareLossColre):
    def _build_loss(self):
        if self.label_manager.ignore_label is not None:
            warnings.warn('Support for ignore label with Skeleton Recall is experimental and may not work as expected')
        topology_kwargs = {
